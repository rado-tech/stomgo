...
export default function ServicesPage() {
  ...
  const addCustom = async () => {
    setSaving(true);
    try {
      await api("/api/clinic/services", { json: custom });
    ...
    <div className="mx-auto max-w-3xl">
      <div className="sticky top-0 z-20 -mx-1 mb-4 flex items-center justify-between rounded-b-2xl bg-white/95 px-1 py-3 backdrop-blur">
        <div>
          ...
          <p className="text-[13px] text-zinc-500">Narxlar so&apos;mda, &quot;dan – gacha&quot; diapazon</p>
        </div>
                ...
                <div key={s.serviceId} className="flex flex-wrap items-center gap-3 border-b border-zinc-50 pb-2 last:border-0">
                  ...
                  {s.enabled && (
                    <div className="flex items-center gap-1.5 text-[13px]">
                      <input
                        type="number" value={s.priceMin || ""} placeholder="dan"
                        onChange={(e) => update(s.serviceId, { priceMin: parseInt(e.target.value, 10) || 0 })}
                        className="w-28 rounded-lg border border-zinc-200 px-2 py-1.5 outline-none focus:border-teal-500"
                      />
                      <span className="text-zinc-400">–</span>
                      <input
                        type="number" value={s.priceMax || ""} placeholder="gacha"
                        onChange={(e) => update(s.serviceId, { priceMax: parseInt(e.target.value, 10) || 0 })}
                        className="w-28 rounded-lg border border-zinc-200 px-2 py-1.5 outline-none focus:border-teal-500"
                      />
                    </div>
                  )}
        ...
        {!customOpen ? (
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <input
                type="number" value={custom.priceMin} placeholder="Narx dan"
                onChange={(e) => setCustom((c) => ({ ...c, priceMin: e.target.value }))}
                className="w-28 rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-[13px] outline-none"
              />
              <span className="text-zinc-400">–</span>
              <input
                type="number" value={custom.priceMax} placeholder="gacha"
                onChange={(e) => setCustom((c) => ({ ...c, priceMax: e.target.value }))}
                className="w-28 rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-[13px] outline-none"
              />
