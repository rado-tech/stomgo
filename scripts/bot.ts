...
const CLINIC_MENU = new Keyboard()
  ...
  .row()
  .text("🔑 Check-in kod").text("ℹ️ Yordam")
  .resized().persistent();
...
bot.on("message:text", async (ctx) => {
  ...
  if (role?.type === "clinic") {
    ...
    if (text === "🔑 Check-in kod") {
      await ctx.reply(
        `🔑 Check-in kod: <code>${role.clinic.checkinCode}</code>\n\nBemor kelganda shu kodni aytishi yoki stoldagi QR kodni skanerlashi mumkin. QR kodni chop etish: panel → Sozlamalar → QR kod.`,
        { parse_mode: "HTML" }
      );
      return;
    }
